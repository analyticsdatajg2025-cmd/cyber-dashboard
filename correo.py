"""
correo.py — PNG + ENVÍO DE CORREO (flujo WhatsApp). Separado a propósito.

Fusiona render.py + enviar_correo.py. No cambia el comportamiento:
  - construir_html(r) -> render_png(html, archivo)  arma la imagen por marca
...
with open("gmail.txt", "r", encoding="utf-8") as f:
    _lineas = [ln.strip() for ln in f if ln.strip()]
GMAIL_USER = _lineas[0]
GMAIL_PASS = _lineas[1].replace(" ", "")
...
def construir_html(r):
    ...
    max_p = max(proy) or 1
    max_r = max(real[:corte + 1], default=1) or 1
    filas = ""
    for h in range(24):
        p = proy[h]
        if h <= corte:
...
if __name__ == "__main__":
    # Prueba local: genera PNG + manda correo usando el cerebro (pipeline.py)
    from pipeline import armar_reporte, MARCAS
    ...
    for marca in MARCAS:
        ...
        reportes.append(r)
        print(f"  OK {marca}  (corte {r['corte']}:00, avance {r['avance']:.1%})")
    print("Enviando correo...")
    enviar(construir_correo(reportes))
